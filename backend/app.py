...
# Prefer an env var in real deployments; falls back for local dev.
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "college_management_2026")

# The React (Vite) dev server runs on its own origin (default
# http://localhost:3000), while Flask runs on http://localhost:5000.
# In dev, Vite's proxy (see vite.config.ts) makes API calls same-origin,
# so no browser CORS round trip is needed. CORS is still enabled here as
# a fallback for anyone hitting the API directly from a different origin
# (e.g. running the built frontend on its own static host).
CORS(
    app,
...
# Session cookie must be sent on cross-site XHR/fetch calls made with
# credentials: 'include'. These settings only matter when the frontend
# is NOT proxied through Vite (i.e. talking cross-origin directly).
app.config.update(
...
db = mysql.connector.connect(
    ...
    password=os.environ.get("DB_PASSWORD", "P@reet@22"),
...
# =========================================================
# ROLE MAPPING (frontend uses capitalized role labels)
# =========================================================

# The React app sends 'Student' | 'Faculty' | 'Administrator' | 'HOD' | 'Librarian'.
# The DB/session logic below uses lowercase keys, with 'admin' for Administrator.
FRONTEND_ROLE_MAP = {
    "student": "student",
    "faculty": "faculty",
    "administrator": "admin",
...
def normalize_role():
    """Read 'role' from the incoming form and map it to backend's internal role key."""
    raw = (request.form.get("role") or "").strip().lower()
    return FRONTEND_ROLE_MAP.get(raw, raw)


def wants_json():
    """The React SPA always calls the API via fetch, so JSON is the default response
    type for POST endpoints. This still lets a plain HTML form fall back if ever used."""
    return True
...
@app.route("/api/me")
def api_me():
    """Lets the SPA restore a logged-in session on page load/refresh."""
    ...
    if role == "student":
        ...
        user.update({
            "id": session.get("hod_id"),
            "name": session.get("hod_name"),
            "department": session.get("department"),
        })
    elif role == "librarian":
        user.update({
            "id": session.get("librarian_id"),
            "name": session.get("librarian_name"),
        })
    ...
    if request.method == "POST":
        ...
        if role == "student":
            ...
            try:
                ...
                return jsonify({"success": True, "message": "Student registration successful", "redirect": "/login"})

            except mysql.connector.Error as e:

                db.rollback()

                return jsonify({"success": False, "message": f"Student registration error: {e}"}), 400
        ...
        # =================================================
            ...
            try:
                ...
                return jsonify({"success": True, "message": "Faculty registration successful", "redirect": "/login"})

            except mysql.connector.Error as e:

                db.rollback()

                return jsonify({"success": False, "message": f"Faculty registration error: {e}"}), 400
        ...
        # =================================================
            ...
            try:
                ...
                return jsonify({"success": True, "message": "Administrator registration successful", "redirect": "/login"})

            except mysql.connector.Error as e:

                db.rollback()

                return jsonify({"success": False, "message": f"Administrator registration error: {e}"}), 400
        ...
        # =================================================
            ...
            try:
                ...
                return jsonify({"success": True, "message": "HOD registration successful", "redirect": "/login"})

            except mysql.connector.Error as e:

                db.rollback()

                return jsonify({"success": False, "message": f"HOD registration error: {e}"}), 400
        ...
        # =================================================
            ...
            try:
                ...
                return jsonify({"success": True, "message": "Librarian registration successful", "redirect": "/login"})

            except mysql.connector.Error as e:

                db.rollback()

                return jsonify({"success": False, "message": f"Librarian registration error: {e}"}), 400
...
def login():
    ...
    if request.method == "POST":
        ...
        if role == "student":
            ...
            return jsonify({"success": False, "message": "Invalid Student Username or Password"}), 401
        ...
        # =================================================
            ...
            return jsonify({"success": False, "message": "Invalid Faculty Username or Password"}), 401
        ...
        # =================================================
            ...
            return jsonify({"success": False, "message": "Invalid Administrator Username or Password"}), 401
        ...
        # =================================================
            ...
            return jsonify({"success": False, "message": "Invalid HOD Username or Password"}), 401
        ...
        # =================================================
            ...
            return jsonify({"success": False, "message": "Invalid Librarian Username or Password"}), 401
